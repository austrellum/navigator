...
В каталог также попадают русскоязычные разделы не-русскоязычных борд и т.п.
...
Одобренной считается борда, на которой присутствует *консистентное общение* — такое общение характеризуется тремя признаками:
...
Борда находится в открытом доступе.
...
Борда не является русскоязычной, но имеет русскоязычные разделы.

*Этот тег отображается только в JSON-версии.*

#### #clone

Намеренное копирование атрибутов других, более успешных проектов: использование идентичного движка, схожих названий, доменов и т.п; явные попытки паразитировать на таких атрибутах.
...
Положительный опыт использования. Адаптивный дизайн, правильная цветовая палитра (или несколько), в целом удобный интерфейс, доступный на большинстве популярных платформ.
