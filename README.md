...
Существующие решения имеют ряд взаимосвязанных недостатков, включая:
  ...
  - *Устаревание* — до сих пор используются фреймы и не-адаптивная верстка (см. [overchan.ru](http://overchan.ru/))
  - *Громоздкость* — список занимает несколько десятков экранов и содержит много ненужной информации (см. [neolurk.org](https://neolurk.org/wiki/%D0%A1%D0%BF%D0%B8%D1%81%D0%BE%D0%BA_%D1%80%D1%83%D1%81%D1%81%D0%BA%D0%BE%D1%8F%D0%B7%D1%8B%D1%87%D0%BD%D1%8B%D1%85_%D0%B8%D0%BC%D0%B8%D0%B4%D0%B6%D0%B1%D0%BE%D1%80%D0%B4))
  - *Предвзятость* — список содержит описания, основанные на взаимных претензиях, разногласиях, местечковых форсах и т.п.
  - *Чрезмерность* — попытки охватить борды всех времен, народов и языков вместе с их разделами и движками (см. [imageboards.json](https://github.com/ccd0/imageboards.json))
  - *Неактуальность* — данные устаревают на несколько лет или больше, сайты в таких каталогах давно закрылась
  - *Закрытость* — каталог тяжело поддерживать (в частности — обновлять) и, при необходимости, переносить
...
Можно оставить ссылку через [обратную связь](#обратная-связь). Если ссылка соответствует [критериям](#какие-ссылки-попадают-в-каталог), то она появится в следующей редакции.
...
Ссылка попадет в каталог, если она соответствует двум условиям:
  ...
  2. Хотя бы один сервис содержит контент на русском языке

В первую очередь в каталог попадают ссылки на русскоязычные борды. Кроме того, в каталог попадают ссылки на связанные ресурсы бордосферы — например, вики-системы или списки ссылок.
...
Новые редакции выходят каждый месяц, первого числа.
...
Приветствуется использование и *ненасильственное распространение* материалов «Навигатора бордосферы».

Под *ненасильственным* понимается распространение в специально предназначенных для этого местах (например, в тредах для пиара) или с явного согласия местной администрации.
...
В отдельных случаях подразумевается полное удаление сайта к следующей редакции.
...
Если сайт имеет несколько адресов, то используется более «активный» тег. Например, если сайт недоступен по адресу example.org, но доступен по адресу example.i2p, то такой сайт будет помечен как #live.
...
### Международные
  ...
  - [chan.city](https://chan.city/) — 2024-05-20
